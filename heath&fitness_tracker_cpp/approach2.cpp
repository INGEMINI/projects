...
Workout getWorkoutInput() {
    ...
    // Clear any leftover newline before reading the string (to ensure getline works)
    cin.ignore();  // Clear the leftover newline after the previous cin

    cout << "Enter workout type (e.g., Running, Swimming): ";
    getline(cin, type);  // Read the workout type string

    cout << "Enter workout duration (in minutes): ";
    cin >> duration;
    cin.ignore();  // Ignore the newline left by reading duration

    cout << "Enter calories burned: ";
    cin >> calories;
    cin.ignore();  // Ignore the newline left by reading calories

    return Workout(type, duration, calories);  // Return the created workout object
}
